...
result <- replicate(R,{
  # Generate data
  simulated_data <- matrix(0,nrow = num_samples,ncol=num_features)
  for(i in 1:(num_samples/2)){
    simulated_data[i,]<- arima.sim(list(order=c(1,0,0), ar=0.5), 
                         n=num_features)[1:num_features]
    simulated_data[i+(num_samples/2),]<- 0.5 + arima.sim(list(order=c(1,0,0), 
                                              ar=0.5),n=num_features)[1:num_features]
  }
  ...
  # Generate data
  test_data <- matrix(0,nrow = 500,ncol=num_features)
  for(i in 1:(500/2)){
    test_data[i,]<- arima.sim(list(order=c(1,0,0), ar=0.5), 
                                   n=num_features)[1:num_features]
    test_data[i+(500/2),]<- 0.5 + arima.sim(list(order=c(1,0,0), 
                                                              ar=0.5),n=num_features)[1:num_features]
  }
  ...
  #result1=eigen(L1);
  #sample1 <- sample_Y_from_eigendecomposition(k=2,result1);
  ...
  #T3 <- system.time({
  #  sample_random1 <- sampleWithoutReplacement((num_samples/2),2);
  #  sample_random2 <- sampleWithoutReplacement((num_samples/2),2);
  ...
  T5 <- system.time({test_prediction <- knn_madd(data_without_label,test_data_without_label,
                                                 simulated_data$class,5,as.matrix(data_without_label))})[3]
