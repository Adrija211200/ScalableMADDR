...
# Create mean vector for both classes
class_means <- matrix(0, nrow = num_classes, ncol = num_features)

# Create variance matrices for each class
class_variances <- list(diag(num_features), 2*diag((num_features)))

# Generate data using multivariate normal distribution with different variances
generate_data <- function(num_samples, class_means, class_variances) {
  ...
  for (i in 1:num_samples) {
    ...
    variance_matrix <- class_variances[[class_label]]
    data[i, ] <- mvrnorm(1, mu = mean_vector, Sigma = variance_matrix)
  }
...
result <- replicate(R,{
  # Generate data
  simulated_data <- generate_data(num_samples, class_means, class_variances)
  ...
  # Generate data
  test_data <- generate_data(num_samples, class_means, class_variances)
  ...
  T1 <- system.time({# Sample from 1st population
    ...
    L1 = as.matrix(simulated_data[simulated_data$class==1,
                                                   -ncol(simulated_data)])%*%t(
  ...
  #T2 <- system.time({
   # sample_random1 <- sampleWithoutReplacement((num_samples/2),16);
    #sample_random2 <- sampleWithoutReplacement((num_samples/2),16);
  ...
  T4<- system.time(traditional_prediction <- knn_madd(data_without_label,
...
result<-result[-1,]
rownames(result) <- c("mis_prob1",#"mis_prob2",
                      #"mis_prob3",
                      "mis_prob4",
                      "T1",#"T2","T3",
                      "T4"
