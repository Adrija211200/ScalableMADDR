  ...
  class_means <- matrix(c(0,0.25), nrow = num_classes,ncol = num_features)
  ...
  result <- replicate(R,{
    ...
    sample_random1 = sample(1:nrow(A1),16,replace = F)
    ...
    sample_random2 = sample(1:nrow(A2),16,replace = F)
    ...
    # Custom KNN function using MADD distance metric
    T1 <- system.time(test_prediction <- 
    ...
    T2 <- system.time(test_prediction_random <- knn_madd_optimized(data_without_label,
