...
class_means <- rep(0,num_features)
#class_means_2 <- rep(0.5,num_features)
#class_means <- rbind(class_means_1,class_means_2)
# Generate data using multivariate normal distribution with varying mean
generate_data <- function(num_samples, class_means) {
  data <- rbind(rmvn(num_samples/2,class_means,3*diag(num_features)),
                rmvt(num_samples/2,class_means,diag(num_features),3))
  return(data)
}
...
result <- replicate(R,{
  ...
  T1 <- system.time({# Sample from 1st population
    ...
    result1=eigen(L1);
    sample1 <- sample_Y_from_eigendecomposition(k=4,result1);
  ...
  T2 <- system.time({
    sample_random1 <- sampleWithoutReplacement((num_samples/2),4);
    sample_random2 <- sampleWithoutReplacement((num_samples/2),4);
