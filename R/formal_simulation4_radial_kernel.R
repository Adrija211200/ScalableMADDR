...
generate_data <- function(n,d) {
  # Set dispersion matrix
  dispersion_matrix <- 0.5 * diag(d)
  
  # Generate data for both populations together
  generate_sample <- function() {
    for ( i in 1:(n/2)){
      # Population 1
      if (runif(1) < 0.5) {
        rnorm(d, mean = 1, sd = sqrt(0.5))
      } else {
        rnorm(d, mean = -1, sd = sqrt(0.5))
      }
    } }
    for 
      # Population 2
      if (runif(1) < 0.5) {
        rnorm(d, mean = rep(c(1, -1), length.out = d), sd = sqrt(0.5))
      } else {
        rnorm(d, mean = rep(c(-1, 1), length.out = d), sd = sqrt(0.5))
      }
    }
  ...
  # Generate data
  all_data <- replicate(n, generate_sample(), simplify = FALSE)
  return(all_data)
}
...
data_together <- generate_multivariate_data_together(d, n)
